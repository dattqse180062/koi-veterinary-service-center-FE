...
interface Feedback {
    ...
    comment: string;
    dateTime: string;
    appointmentID: number; // Liên kết với appointment
}
...
const FeedbackAndRatingTable: React.FC = () => {
    ...
    const handleDelete = () => {
        if (selectedFeedback) {
            setOrderData((prevData) => prevData.filter((order) => order.appointmentID !== selectedFeedback.appointmentID));
    ...
    const getAppointmentDetails = (appointmentID: number): Appointment | undefined => {
        return appointmentData.find((appointment) => appointment.appointmentID === appointmentID);
    };
                ...
                <tbody>
                    {currentOrders.map((feedback) => (
                        <tr key={feedback.feedbackId}>
                            <td className="fw-bold">{feedback.feedbackId}</td>
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ...
                                <p><strong>Date & Time:</strong> {selectedFeedback.dateTime}</p>

                                {/* Hiển thị chi tiết từ bảng Appointment */}
                                {getAppointmentDetails(selectedFeedback.appointmentID) ? (
                                    <>
                                        <p><strong>Appointment ID:</strong> {getAppointmentDetails(selectedFeedback.appointmentID)?.appointmentID}</p>
                                        <p><strong>Created Date:</strong> {getAppointmentDetails(selectedFeedback.appointmentID)?.created}</p>
                                        <p><strong>Customer Name:</strong> {getAppointmentDetails(selectedFeedback.appointmentID)?.customer}</p>
                                        <p><strong>Total Price:</strong> ${getAppointmentDetails(selectedFeedback.appointmentID)?.totalPrice.toFixed(2)}</p>
                                        <p><strong>Description:</strong> {getAppointmentDetails(selectedFeedback.appointmentID)?.description}</p>
                                        <p><strong>Status:</strong> {getAppointmentDetails(selectedFeedback.appointmentID)?.status}</p>
                                    </>
                                ) : (
                ...
                <div className="modal fade show" style={{ display: 'block', backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <p>Are you sure you want to delete feedback ID: {selectedFeedback?.feedbackId}?</p>
                            </div>
                            <div className="modal-footer">
